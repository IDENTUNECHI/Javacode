...
public class TicTacToe4 {
    ...
    static boolean won(char[][] board, int x, int y) {
        char player = board[x][y]; // 현재 위치의 돌을 놓은 플레이어 ('X' 또는 'O')
        boolean allMatch = true;

        // 돌을 놓은 '행'이 모두 같은 플레이어의 돌인지 검사한다.
        for (int j = 0; j < 3; j++) {
            if (board[x][j] != player) {
                allMatch = false;
                break;
        ...
        // 돌을 놓은 '열'이 모두 같은 플레이어의 돌인지 검사한다.
        allMatch = true;
        for (int i = 0; i < 3; i++) {
            if (board[i][y] != player) {
                allMatch = false;
        ...
        // 돌을 놓은 '대각선'이 모두 같은 플레이어의 돌인지 검사한다.
        if (x == y) {
            allMatch = true;
            for (int i = 0; i < 3; i++) {
                if (board[i][i] != player) {
        ...
        // 돌을 놓은 '엇대각선'이 모두 같은 플레이어의 돌인지 검사한다.
        if (x == 2 - y) {
            allMatch = true;
            for (int i = 0; i < 3; i++) {
